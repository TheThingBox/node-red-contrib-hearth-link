module.exports = function(RED) {
  ...
  function normalizeTopic(topic){
    ...
    return newStr.replace(/[^a-zA-Z0-9_]/g,'_');
  }

  function cipher(plainMsg, key){
    let encrypted
    if(!key){
      key = 'hearth_link_default_key'
    }
    try{
      plainMsg = JSON.stringify(plainMsg)
    }catch(e){}
    ...
    if(this.brokerConn) {
      ...
      if(checkSubTopic(this.topic)) {
        ...
        this.brokerConn.subscribe(this.topic, this.qos, function(topic, payload, packet) {
          ...
          var plainText = decipher(payload, node.key);
          if(plainText !== null){
            node.send(plainText);
          }
        }, this.id);
  ...
  RED.nodes.registerType('hearth-link-in', hearthLinkIn);

  function hearthLinkOut(config) {
    RED.nodes.createNode(this, config);
    this.topic = normalizeTopic(config.topic);
    this.key = config.key || "";
    this.qos = parseInt(config.qos);
  ...
  RED.nodes.registerType('hearth-link-out', hearthLinkOut);
};
